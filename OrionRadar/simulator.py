...
from adsb_track import SURFACE_REF, UDP_IP, UDP_PORT

SURFACE_LAT_MAX = SURFACE_REF[0] + 0.002
SURFACE_LAT_MIN = SURFACE_REF[0] - 0.002
SURFACE_LON_MAX = SURFACE_REF[1] + 0.002
SURFACE_LON_MIN = SURFACE_REF[1] - 0.002
...
def encode_cpr(lat, lon, t_odd, is_surface=False):
    ...
    # Standard CPR encoding algorithm
...
def generate_surface_position(icao, lat, lon, t_odd, speed, heading):
    ...
    if speed > 0 and speed < 0.125:
        ...
        movement = int(speed) + 2  # Simplified mapping
    else:
        movement = 124
...
class Aircraft:
    ...
    def update(self, dt):
        ...
        lat_movement = (
            (self.speed / 3600.0 * dt) * math.cos(math.radians(self.heading)) / 60.0
        )
        lon_movement = (
            (self.speed / 3600.0 * dt)
            * math.sin(math.radians(self.heading))
            / (60.0 * max(0.1, math.cos(math.radians(self.lat))))
        )
        ...
        if self.is_surface:
            if self.lat > SURFACE_LAT_MAX:
                self.lat = SURFACE_LAT_MAX
                self.heading = (self.heading + 180) % 360
            elif self.lat < SURFACE_LAT_MIN:
                self.lat = SURFACE_LAT_MIN
                self.heading = (self.heading + 180) % 360

            if self.lon > SURFACE_LON_MAX:
                self.lon = SURFACE_LON_MAX
                self.heading = (self.heading + 180) % 360
            elif self.lon < SURFACE_LON_MIN:
                self.lon = SURFACE_LON_MIN
                self.heading = (self.heading + 180) % 360
...
def main():
    ...
    try:
        with open("aircraft_database.csv", "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if row["category"] == "AIRBORNE":
                    db_airborne.append((icao, callsign))
                else:
                    db_ground.append((icao, callsign))
    except (OSError, csv.Error, KeyError, ValueError, UnicodeDecodeError):
    ...
    for i in range(min(10, len(db_airborne))):
        ...
        aircraft_list.append(
            Aircraft(icao, lat, lon, alt, speed, heading, False, callsign, ver)
        )

    # All Surface entities (Airplanes and Vehicles)
    for i in range(len(db_ground)):
        icao, callsign = db_ground[i]
        lat = SURFACE_REF[0] + random.uniform(-0.002, 0.002)
        lon = SURFACE_REF[1] + random.uniform(-0.002, 0.002)
        alt = 0
        speed = random.choice([0, 0, 10, 15])  # Parked or taxiing
        heading = random.randint(0, 359)
        ver = random.choice([0, 1, 2])
        aircraft_list.append(
            Aircraft(icao, lat, lon, alt, speed, heading, True, callsign, ver)
        )

    if os.environ.get("ORION_MANAGED") != "1":
        print(f"simulator → {UDP_IP}:{UDP_PORT}")
    try:
        while True:
            dt = 0.5  # 2 Hz tick
            for ac in aircraft_list:
                ...
                if ac.is_surface:
                    msgs.append(
                        generate_surface_position(
                            ac.icao, ac.lat, ac.lon, ac.t_odd, ac.speed, ac.heading
                        )
                    )
                    if ac.tick % 10 == 0:  # Every 5 seconds
                        msgs.append(
                            encode_callsign(ac.icao, ac.callsign, tc=4, ca=1)
                        )  # light surface
                        msgs.append(
                            encode_status(ac.icao, version=ac.adsb_version, subtype=1)
                        )
                else:
                    msgs.append(
                        generate_airborne_position(
                            ac.icao, ac.lat, ac.lon, ac.alt, ac.t_odd
                        )
                    )
                    msgs.append(
                        generate_airborne_velocity(ac.icao, ac.speed, ac.heading)
                    )
                    if ac.tick % 10 == 0:  # Every 5 seconds
                        msgs.append(
                            encode_callsign(ac.icao, ac.callsign, tc=4, ca=5)
                        )  # heavy airborne
                        msgs.append(
                            encode_status(ac.icao, version=ac.adsb_version, subtype=0)
                        )
