...
Thinking about agility, we created a master script that does all the heavy lifting for you in just a single command!

Open your terminal in the project folder and first install the dependencies:
```bash
pip install pandas pyModeS Flask
```

Next, run the master script:
```bash
python run.py
```
...
## 📦 Dependencies
The system relies on the `pandas` and `pyModeS` libraries for data processing and ADS-B positional encryption, as well as the `Flask` framework. Make sure they are installed in your Python environment.
