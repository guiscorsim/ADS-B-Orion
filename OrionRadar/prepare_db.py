...
def build_anac_json(row):
    try:
        ...
        ops = []
    try:
        props = json.loads(row["PROPRIETARIOS"])
    except (json.JSONDecodeError, TypeError, ValueError):
        props = []
    ...
    prop_doc = (
        props[0]["DOCUMENTO"] if len(props) > 0 and "DOCUMENTO" in props[0] else "N/A"
    )
    ...
    data = {
        ...
        "pmd": str(row["NR_PMD"]),
        "situacao": "MATRÍCULA CANCELADA"
        if pd.notna(row["DT_CANC"])
        else "SITUAÇÃO NORMAL",
        "restricao": str(row["DS_MOTIVO_CANC"])
        if pd.notna(row["DS_MOTIVO_CANC"])
        else "N/A",
        "gravame": str(row["DS_GRAVAME"]),
        "validade_cva": str(row["DT_VALIDADE_CVA"]),
        "operador_tipo": str(row["TP_OPERACAO"]),
        "ca_tipo": str(row["TP_CA"]),
        "proposito": str(row["CD_PROPOSITO_CAVE"])
        if pd.notna(row["CD_PROPOSITO_CAVE"])
        else "N/A",
...
def append_sampled_aircraft(i: int, category: str) -> None:
    ...
    out_rows.append(
        [
            ...
            row["DS_MODELO"],
            get_op(row),
            safe_int_str(row["NR_ANO_FABRICACAO"]),
            row["TP_MOTOR"],
            category,
            row["NR_SERIE"],
            safe_int_str(row["NR_PASSAGEIROS_MAX"]),
            row["DS_GRAVAME"],
            row["DT_VALIDADE_CVA"],
            build_anac_json(row),
        ]
...
for i, category in enumerate(["AIRBORNE"] * 10 + ["GROUND_AIRCRAFT"] * 5):
    append_sampled_aircraft(i, category)
